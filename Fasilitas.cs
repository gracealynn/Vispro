...
namespace Daftar_fasilitas
{
    public partial class Fasilitas : Form
    {
        ...
        private void UpdateBtn_Click(object sender, EventArgs e)
        {
            try
            {
                query = string.Format("UPDATE `info_fasilitas` SET `Fasilitas`='{0}','Asrama'='{1}',`Jumlah`='{2}','Status' = '{3}' where Nomor_Kamar ='{4}'", TxtFasilitas.Text, TxtAsrama.Text, TxtJumlah.Text, StatusCBox.Text, TxtNomorKamar.Text);
                ds.Clear();
                koneksi.Open();
                perintah = new MySqlCommand(query, koneksi);
                adapter = new MySqlDataAdapter(perintah);
